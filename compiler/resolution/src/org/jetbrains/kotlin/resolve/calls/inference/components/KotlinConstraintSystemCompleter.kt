...
class KotlinConstraintSystemCompleter(
    ...
    data class ParameterTypesInfo(
        ...
        companion object {
            val EMPTY = ParameterTypesInfo(null, null, Annotations.EMPTY, isSuspend = false, isNullable = false)
        }
    }
    ...
    private fun Context.collectParameterTypes(argument: PostponedAtomWithRevisableExpectedType): ParameterTypesInfo? {
    ...
    private fun Context.extractParameterTypesInfoFromConstraints(expectedTypeVariable: TypeConstructor): ParameterTypesInfo {
        if (expectedTypeVariable !in notFixedTypeVariables)
            return ParameterTypesInfo.EMPTY
        ...
        return ParameterTypesInfo(
            null,
            foundFunctionalTypes.map { type -> type.arguments.dropLast(1).map { it.type } }.toSet(),
            Annotations.create(foundFunctionalTypes.map { it.annotations }.flatten()),
            foundFunctionalTypes.isNotEmpty() && foundFunctionalTypes.any { it.isSuspendFunctionTypeOrSubtype },
    ...
    private fun extractParameterTypesFromDeclaration(atom: ResolutionAtom) =
        when {
            atom is FunctionExpression && atom.receiverType != null -> listOf(atom.receiverType) + atom.parametersTypes.map { it }
            atom is LambdaKotlinCallArgument -> atom.parametersTypes?.map { it }
            else -> null
        }
    ...
    private fun NewConstraintSystem.createTypeVariablesForParameters(
        argument: PostponedAtomWithRevisableExpectedType,
        parameterTypes: List<List<KotlinType?>>
    ): List<TypeProjection>? {
        val atom = argument.atom
    ...
    private fun Context.collectParameterTypesAndBuildNewExpectedTypes(
         ...
         *
         * It can be so for lambdas: for example, an extension function type can appear in full mode (it may not be known in partial mode).
         */
        val suitablePostponedArguments =
            if (completionMode == ConstraintSystemCompletionMode.PARTIAL) {
    ...
    private fun NewConstraintSystem.buildNewFunctionalExpectedType(
        ...
        val parametersFromDeclaration = prependReceiverTypeIfItIsExtensionFunction(
            parameterTypesInfo.parametersFromDeclaration,
            parameterTypesInfo.parametersFromConstraints,
            parameterTypesInfo.annotations
        )
        val allParameterTypes = (parametersFromConstraints.orEmpty() + parametersFromDeclaration).filterNotNull()
    ...
    private fun Context.fixVariablesInsideType(
        ...
        val isProperTypeConstructorForVariable = typeConstructor is TypeVariableTypeConstructor && typeConstructor in notFixedTypeVariables
